...
class DBService {
    ...
    try {
      ...
      for (const email of Array.from(emailsToTry)) {
        try {
          ...
          lastAuthError = e;
          // handleNetworkError はループ外で一括で行うか、ここで個別に投げる
      ...
      const { data: profile, error: fetchError } = await supabase.from('users')
        .select('*')
        .eq('auth_uid', authUid)
        .maybeSingle();
      ...
      if (!profile && !normalizedLoginId.includes('@')) {
        const { data: linkedProfile, error: updateError } = await supabase.from('users')
  ...
  async completeRegistration(
    ...
    // Supabase Functions を使用して登録を完了する
    try {
      const { data: result, error: funcError } = await supabase.functions.invoke('agency-complete-registration', {
        body: {
  ...
  async logout(): Promise<void> {
    ...
    try {
      ...
      // ログアウト時のネットワークエラーは致命的ではないが、一応ハンドルする
      if (e.message?.includes('fetch') || e.name === 'TypeError') {
        this.handleNetworkError(e);
      }
    }
