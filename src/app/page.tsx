        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Link href="/cases" className="text-primary text-sm font-medium hover:underline">すべて見る</Link>
            </div>
            <Card>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-6 space-y-4">
              <QuickActionButton href="/cases/new" icon="fa-plus-circle" label="新規案件作成" sub="代理店様による案件登録" />
              {user?.role === 'admin' && (
                <>
                  <QuickActionButton href="/agencies" icon="fa-users" label="代理店管理" sub="承認・紹介関係の設定" />
                  <QuickActionButton href="/audit-logs" icon="fa-shield-halved" label="監査ログ閲覧" sub="操作履歴の確認" />
                </>
              )}
            </Card>
          </div>
...
const QuickActionButton = ({ href, icon, label, sub }: { href: string; icon: string; label: string; sub: string }) => (
  <Link href={href} className="flex items-center p-3 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-primary dark:hover:border-primary-dark hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all group">
